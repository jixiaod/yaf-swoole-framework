...
class Ini extends IR_Config
{
     ...
     *      hostname = staging
     *
     * after calling $data = new IR_Config_Ini($file, 'staging'); then
     *      $data->hostname === "staging"
     *      $data->db->connection === "database"
     *
     * The $options parameter may be provided as either a boolean or an array.
     * If provided as a boolean, this sets the $allowModifications option of
     * IR_Config. If provided as an array, there are three configuration
    ...
    public function __construct($filename, $section = null, $options = false)
    {
        if (empty($filename)) {
             ...
             */
            throw new IR_Config_Exception('Filename is not set');
        }
        ...
        if (null === $section) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($iniArray[$sectionName])) {
                     ...
                     * @see IR_Config_Exception
                     */
                    throw new IR_Config_Exception("Section '$sectionName' cannot be found in $filename");
                }
     ...
     * handler to convert any loading errors into a IR_Config_Exception
     *
     * @param string $filename
     * @throws IR_Config_Exception
     * @return array
    ...
    protected function _parseIniFile($filename)
    {
        ...
        if ($this->_loadFileErrorStr !== null) {
             ...
             * @see IR_Config_Exception
             */
            throw new IR_Config_Exception($this->_loadFileErrorStr);
        }
        ...
        foreach ($loaded as $key => $data)
        {
            ...
            switch (count($pieces)) {
                ...
                default:
                     ...
                     * @see IR_Config_Exception
                     */
                    throw new IR_Config_Exception("Section '$thisSection' may not extend multiple sections in $filename");
            }
        }
        ...
        foreach ($thisSection as $key => $value) {
            if (strtolower($key) == ';extends') {
                if (isset($iniArray[$value])) {
                     ...
                     * @see IR_Config_Exception
                     */
                    throw new IR_Config_Exception("Parent section '$section' cannot be found");
                }
            } else {
        ...
        if (strpos($key, $this->_nestSeparator) !== false) {
            ...
            if (strlen($pieces[0]) && strlen($pieces[1])) {
                if (!isset($config[$pieces[0]])) {
                     ...
                     * @see IR_Config_Exception
                     */
                    throw new IR_Config_Exception("Cannot create sub-key for '{$pieces[0]}' as key already exists");
                }
                 ...
                 * @see IR_Config_Exception
                 */
                throw new IR_Config_Exception("Invalid key '$key'");
            }
        } else {
            $config[$key] = $value;
        }
