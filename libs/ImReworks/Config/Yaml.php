...
class Yaml extends IR_Config
{
    ...
    public function setYamlDecoder($yamlDecoder)
    {
        if (!is_callable($yamlDecoder)) {
            throw new IR_Config_Exception('Invalid parameter to setYamlDecoder() - must be callable');
        }
    ...
    public function __construct($yaml, $section = null, $options = false)
    {
        ...
        if (null === $config) {
            // decode failed
            throw new IR_Config_Exception("Error parsing YAML data");
        }
        ...
        if (null === $section) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($config[$sectionName])) {
                    throw new IR_Config_Exception(sprintf(
     ...
     * @return array
     * @throws IR_Config_Exception When $section cannot be found
     */
    protected function _processExtends(array $data, $section, array $config = array())
    {
        if (!isset($data[$section])) {
            throw new IR_Config_Exception(sprintf('Section "%s" cannot be found', $section));
        }
