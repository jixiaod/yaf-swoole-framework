...
use ImReworks\Db\Expr as IR_Db_Expr;
use ImReworks\Db\Adapter\AdapterAbstract as IR_Db_Adapter_Abstract;
...
class Select
{
    ...
    /**
     * IR_Db_Adapter_Abstract object.
     *
     * @var IR_Db_Adapter_Abstract
     */
    protected $_adapter;
     ...
     * Class constructor
     *
     * @param IR_Db_Adapter_Abstract $adapter
     */
    public function __construct(IR_Db_Adapter_Abstract $adapter)
    {
     ...
     * table.
     *
     * The second parameter can be a single string or IR_Db_Expr object,
     * or else an array of strings or IR_Db_Expr objects.
     *
     * The first parameter can be null or an empty string, in which case
     * no correlation name is generated or prepended to the columns named
     * in the second parameter.
     *
     * @param  array|string|IR_Db_Expr $name The table name or an associative array
     *                                         relating correlation name to table name.
     * @param  array|string|IR_Db_Expr $cols The columns to select from this table.
     ...
     *
     * The parameter can be a single string or IR_Db_Expr object,
     * or else an array of strings or IR_Db_Expr objects.
     *
     * @param  array|string|IR_Db_Expr $cols The columns to select from this table.
    ...
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
             ...
             * @see DbException
             */
            throw new DbException("No table has been specified for the FROM clause");
        }
    ...
    public function union($select = array(), $type = self::SQL_UNION)
    {
        ...
        if (!in_array($type, self::$_unionTypes)) {
            throw new DbException("Invalid union type '{$type}'");
        }
     ...
     *
     * @param  array|string|IR_Db_Expr $name The table name.
    ...
    public function getPart($part)
    {
        ...
        if (!array_key_exists($part, $this->_parts)) {
            throw new DbException("Invalid Select part '$part'");
        }
     ...
     * @return PDO_Statement|implode\Db\Statement
     */
    ...
    /**
     * Gets the IR_Db_Adapter_Abstract for this
     * particular Select object.
     *
     * @return IR_Db_Adapter_Abstract
     */
     ...
     * @param  array|string|IR_Db_Expr $name Table name
     * @param  string $cond Join on this condition
     * @param  array|string $cols The columns to select from the joined table
     * @param  string $schema The database name to specify, if any.
     * @return Select This Select object
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        ...
        if (count($this->_parts[self::UNION])) {
            throw new DbException("Invalid use of table with " . self::SQL_UNION);
        }
        ...
        } elseif ($name instanceof IR_Db_Expr|| $name instanceof Select) {
            $tableName = $name;
        ...
        if (!empty($correlationName)) {
            if (array_key_exists($correlationName, $this->_parts[self::FROM])) {
                 ...
                 * @see DbException
                 */
                throw new DbException("You cannot define a correlation name '$correlationName' more than once");
            }
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        if (empty($this->_parts[self::FROM])) {
            throw new DbException("You can only perform a joinUsing after specifying a FROM table");
        }
    ...
    protected function _tableCols($correlationName, $cols, $afterCorrelationName = null)
    {
        ...
        foreach (array_filter($cols) as $alias => $col) {
            ...
            if (is_string($col)) {
                ...
                // Check for columns that look like functions and convert to IR_Db_Expr
                if (preg_match('/\(.*\)/', $col)) {
                    $col = new IR_Db_Expr($col);
    ...
    {
        if (count($this->_parts[self::UNION])) {
            throw new DbException("Invalid use of where clause with " . self::SQL_UNION);
        }
    ...
    protected function _renderColumns($sql)
    {
        ...
        foreach ($this->_parts[self::COLUMNS] as $columnEntry) {
            ...
            if ($column instanceof IR_Db_Expr) {
                $columns[] = $this->_adapter->quoteColumnAs($column, $alias, true);
            } else {
                if ($column == self::SQL_WILDCARD) {
                    $column = new IR_Db_Expr(self::SQL_WILDCARD);
                    $alias = null;
                }
                if (empty($correlationName)) {
                    $columns[] = $this->_adapter->quoteColumnAs($column, $alias, true);
     ...
     * @param string $method
     * @return Select
     * @throws DbException If an invalid method is called.
     */
        ...
        if (preg_match('/^join([a-zA-Z]*?)Using$/', $method, $matches)) {
            ...
            if ($type) {
                ...
                if (!in_array($type, self::$_joinTypes)) {
                    throw new DbException("Unrecognized method '$method()'");
                }
                if (in_array($type, array(self::CROSS_JOIN, self::NATURAL_JOIN))) {
                    throw new DbException("Cannot perform a joinUsing with method '$method()'");
                }
            } else {
