...
use ImReworks\Db\Db as IR_Db;
use ImReworks\Db\Select as IR_Db_Select;
use ImReworks\Db\Statement\StatementInterface;
use ImReworks\Db\Statement\StatementException;
...
abstract class Statement implements StatementInterface
{
     ...
     * @var integer
     */
    protected $_fetchMode = IR_Db::FETCH_ASSOC;
     ...
     * @param mixed $sql Either a string or IR_Db_Select.
     */
    public function __construct($adapter, $sql)
    {
        $this->_adapter = $adapter;

        if ($sql instanceof IR_Db_Select) {
            $sql = $sql->assemble();
        }
    ...
    {
        $data = array();
        if ($style === IR_Db::FETCH_COLUMN && $col === null) {
            $col = 0;
        }
        if ($col === null) {
    ...
    public function fetchColumn($col = 0)
    {
        $data = array();
        $col = (int) $col;
        $row = $this->fetch(IR_Db::FETCH_NUM);
        if (!is_array($row)) {
            return false;
        }
        return $row[$col];
    ...
    public function fetchObject($class = 'stdClass', array $config = array())
    {
        ...
        $row = $this->fetch(IR_Db::FETCH_ASSOC);
        if (!is_array($row)) {
            return false;
        }
    ...
    {
        switch ($mode) {
            case IR_Db::FETCH_NUM:
            case IR_Db::FETCH_ASSOC:
            case IR_Db::FETCH_BOTH:
            case IR_Db::FETCH_OBJ:
                $this->_fetchMode = $mode;
                break;
            case IR_Db::FETCH_BOUND:
            default:
                $this->closeCursor();
                /**
                 * @see StatementException
     ...
     * particular IR_Db_Statement object.
     *
