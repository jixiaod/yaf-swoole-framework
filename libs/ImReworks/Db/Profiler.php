...
use ImReworks\Db\ProfilerQuery;
...
class Profiler
{
     ...
     * @param  integer $queryType   OPTIONAL Type of query, one of the Profiler::* constants
     * @return integer|null
     */
    ...
    public function queryEnd($queryId)
    {
        ...
        if (!isset($this->_queryProfiles[$queryId])) {
            throw new DbException("Profiler has no query with handle '$queryId'.");
        }
        ...
        if ($qp->hasEnded()) {
            throw new DbException("Query with profiler handle '$queryId' has already ended.");
        }
    ...
    public function getQueryProfile($queryId)
    {
        if (!array_key_exists($queryId, $this->_queryProfiles)) {
            throw new DbException("Query handle '$queryId' not found in profiler log.");
        }
